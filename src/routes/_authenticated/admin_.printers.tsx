...
export const Route = createFileRoute("/_authenticated/admin_/printers")({
  head: () => ({
    meta: [
      { title: "Printers · Infinity Techub PMS" },
  ...
  async function testPrint(p: any) {
    try {
      if (p.kind === "printnode" && p.printnode_id) {
        // Simple text via raw
        const encoded = btoa("Infinity Techub PMS — Test print\n\n\n\n");
        ...
        const bytes = buildReceipt("TEST RECEIPT", [
          { type: "text", text: "Infinity Techub PMS", align: "center" },
...
function JobsTab({ propertyId: _p }: { propertyId: string }) {
  ...
  function browserFallback() {
    browserPrint(`<h1>Infinity Techub PMS</h1><p>Sample document sent via the browser print dialog.</p>
